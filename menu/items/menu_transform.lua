...
function zMenuClass:updateMenuSize(x,y)
    ...
    self.feature_panel:bitmap({name = "background_texture",texture = "guis/textures/background_pattern", texture_rect = {0,0,self.feature_panel:w()/mul,self.feature_panel:h()/mul},x = 4,y = 4,w = self.feature_panel:w() - 8,h = self.feature_panel:h() - 8,layer = 1})
    self.left_side_panel:grow(0,new_y)
    self.left_side_panel:remove(self.left_side_panel:child("background_texture"))
    self.left_side_panel:bitmap({name = "background_texture",texture = "guis/textures/background_pattern", texture_rect = {0,0,self.left_side_panel:w()/mul,self.left_side_panel:h()/mul},x = 4,y = 4,w = self.left_side_panel:w() - 8,h = self.left_side_panel:h() - 8,layer = 1})
